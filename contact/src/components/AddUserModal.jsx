...
const AddModal = ({setResponse})=>{
      ...
      async  function addUserHandler (e){
        ...
        if(userData.name===""||userData.email===""||userData.password===''){
         ...
         closeModal()
         const  {data} = await axios.post('http://localhost:4000/api/contacts',userData)
    ...
    <div>
     <div className="text-2xl font-extrabold ">
      <button  className="bg-black text-white p-3 rounded-lg mt-2 ml-10" onClick={openModal}>ADD USER</button>
      </div>
      <Modal
        isOpen={modalIsOpen}
